...
module.exports = {
    ...
    try {
      if (! args [0]) {
        let embed = new Discord.MessageEmbed ()
          ...
          .addField ("delete [ID]", "Delete a specific error")
      ...
      switch (args [0]) {
        ...
        case "delete": {
          ...
          if (args [1]) {
            ...
            if (! deleted) {
              message.channel.send ("No errors found with that ID");
            }
            return;
          }
          ...
          if (errors.length> 0) {
            errors.forEach (error => {
              let pull = require (`../../ errors / $ {error}`);
              errorsString + = `$ {++ i} - $ {pull.thisfile} \ n`;
            });
          } else {
          ...
          message.channel.send (embederrors)
            .then (m => {
              ...
              collectorDelete.on ("collect", msg => {
                ...
                message.channel.send (`\` $ {errors [parseInt (msg.content) - 1]} \ `deleted`);
                return;
              })
            });
          break;
        ...
        case "list": {
          ...
          errorsArray.forEach (er => {embedListErrors.addField (`$ {i ++} - $ {er.thisfile}`, `$ {er.msg} \ n \ nDate: $ {er.date} \ nID: $ {er .errorID} `);});
  ...
  config: {
    name: "error",
    aliases: ["err", "and"],
    description: "Check the errors saved by the error handler",
    usage: "error",
